...
UPDATE wordform
SET wordform_stressed =
    stress_last_syllable(wordform_stressed, find_nth_stressed_syllable_rev(wordform_stressed, 1) + 1)
WHERE lemma_id IN
    (
...
UPDATE wordform
SET wordform_stressed =
    stress_last_syllable(wordform_stressed, find_nth_stressed_syllable_rev(wordform_stressed, 1) + 2)
WHERE lemma_id IN
    (
        WITH RECURSIVE ctx(lemma_id) AS (
...
UPDATE wordform
SET wordform_stressed =
    stress_last_syllable(wordform_stressed, num_syllables)
...
UPDATE wordform
SET wordform_stressed = stress_last_syllable(wordform_stressed, num_syllables)
WHERE lemma_id IN (
    SELECT lemma_id FROM wordform WHERE (wordform LIKE '%ена' OR wordform LIKE '%еса') AND tag = 'Ncnpi'
) AND tag = 'Ncnpi';

UPDATE wordform
SET wordform_stressed = stress_last_syllable(wordform_stressed, num_syllables - 1)
WHERE lemma_id IN (
    SELECT lemma_id FROM wordform WHERE (wordform LIKE '%ена' OR wordform LIKE '%еса') AND tag = 'Ncnpi'
) AND tag = 'Ncnpd';
...
UPDATE wordform
SET wordform_stressed = stress_last_syllable(wordform_stressed, CASE WHEN tag LIKE '%i' THEN num_syllables - 1 ELSE num_syllables - 2 END)
...
UPDATE wordform
SET wordform_stressed 
    = stress_last_syllable(wordform_stressed, MIN(num_syllables, (SELECT find_nth_stressed_syllable_rev(lemma_stressed, 1) FROM lemma WHERE lemma_id = wordform.lemma_id) + 1))
