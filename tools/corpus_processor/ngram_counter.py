...
def worker_process(input_queue, output_queue, n, min_freq, files_per_cycle, interning):
    try:
        counter = Counter()
        ngram_fun = lambda x: x
        if n > 1:
            ngram_fun = lambda x: ngrams(x, n=n)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--files-per-cycle', default=500, type=int)
    ...
    for i in range(num_processes):
        ...
        process = Process(target=worker_process,
                          args=(iq, oq, args.n, args.min_freq_per_cycle, args.files_per_cycle, args.interning))
        process.daemon = True
        process.start()
